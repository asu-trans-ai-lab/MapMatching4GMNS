...
double g_GetPoint2LineDistance(const GDPoint* pt, const GDPoint* FromPt, const GDPoint* ToPt, double UnitMile, bool no_intersection_requirement)
{
  double U;
  ...
  if (no_intersection_requirement == false)
  {
    ...
    if (U < 0.0 || U > 1.0)
      return UnitMile * 100 > 999999 ? UnitMile * 100 : 999999;
    // max(UnitMile * 100, 999999); // intersection does not fall within the segment
  }
...
public:
  ...
  */
  // Reference the Dr.Peiheng, 02/02/21, consider using move later
  //A good example to apply the move semantics is AllocateVector() in main_api.cpp in DTALite.
  ...
  void AllocatePathNodeVector(const std::vector<int>& node_vector, bool backwardflag = false)
  {
...
public:
  ...
  void BuildGridSystem()
  {
    ...
    if (MAX_GRID_SIZE_ < g_node_vector.size() / 3000)
    {
      g_grid_size = MAX_GRID_SIZE_;
    }
    else if (MAX_GRID_SIZE_ >= g_node_vector.size() / 3000)
    {
  ...
  bool bInsideGrid(const GDPoint* pt)
  {
  ...
  void AlignGPSPoints2Route(int agent_no)
  {
    ...
    for (int i = 1; i < g_agent_vector[agent_no].m_node_size - 1; i++) // for each link along the route
    {
      ...
      double least_distance_to = MAX_LABEL_COST_; // init
      ...
      for (int g = 0; g < m_GridMatrix[x_i][y_i].m_GPSPointVector.size(); g++) // for each GPS point of an agent in the cell
      {
        ...
        for (int p = 0; p < g_link_vector[link_no].m_PointVector.size() - 1; p++)
        {
          ...
          if (g_link_vector[link_no].to_node_id == 172)
          {
            TRACE("grid: %d,%d,trace_d: %d, dist = %f\n",
              x_i, y_i,
              m_GridMatrix[x_i][y_i].m_GPSPointVector[g].trace_id, distance_to);
          }
          */
  ...
  void find_path_for_agents_assigned_for_this_thread()
  {
    ...
    for (int i = 0; i < m_agent_vector.size(); i++)
    {
      ...
      p_agent->AllocatePathNodeVector(l_node_size, temp_path_node_vector, false);
...
void g_ReadInputData(int mode)
{
  ...
  if (mode == 1 && gps_parser.OpenCSVFile("input_agent.csv", true))
  {
    double x, y;
    string time_stamp;
    int time_in_second;
  ...
  else
  {
    ...
    gps_point_count = 0;
    if (mode == 0 && gps_parser.OpenCSVFile("trace.csv", true))
    {
...
void g_OutputLinkPerformanceCSVFile()
{
  ...
  FILE* g_pFileLinkPerformance = nullptr;
    ...
    for (int a = 0; a < g_agent_vector.size(); a++)
    {
      ...
      for (int i = 1; i < p_agent->m_node_size - 1; i++)
      {
        ...
        if (p_agent->downstream_node_matched_time_in_sec.find(link_no) != p_agent->downstream_node_matched_time_in_sec.end())
        {
          ...
          int delay_temp = travel_time_in_second - g_link_vector[link_no].FFTT_in_sec;
          int delay = delay_temp > 0 ? delay_temp : 0;
...
{
  cout << "To avoid complex data folder settings, please always first put the input data on the current directory" << endl;
  cout << "Two modes:" << endl;
  cout << "modes 0: reading trace.csv, then automaticically generate input_agent.csv and output agent.csv" << endl;
  cout << "model 1: reading input_agent.csv, then output agent.csv and link_performance.csv" << endl;

  clock_t start_t, end_t, total_t;

  g_ReadInputData(mode);
  start_t = clock();
