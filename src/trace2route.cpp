...
float g_GridResolution = 0.02;  // in terms of long/lat 
...
public:
	...
	void IdentifyNetworkONode(int agent_no)
	{
		...
		if (g_agent_vector[agent_no].o_node_no >= 0)  // with user prespecified origin 
		{
		...
		for (int l = 0; l < g_link_vector.size(); l++) // for all links in this cell
		{
			...
			if (g_link_vector[l].from_node_id == 35493 && g_link_vector[l].to_node_id == 35486)
				TRACE("%d->%d\n", g_link_vector[l].from_node_id, g_link_vector[l].to_node_id);

			if (g_link_vector[l].from_node_id == 35485 && g_link_vector[l].to_node_id == 35492)
				TRACE("%d->%d\n", g_link_vector[l].from_node_id, g_link_vector[l].to_node_id);
			...
			{ // case of good conditioning
				TRACE("case of good conditioning\n");
			...
			// rule 2: we check this relative angle condition for both ill and good conditions,
			...
			for (int y_i = 0; y_i <= g_grid_size; y_i++)
			{
				...
				if (m_GridMatrix[x_i][y_i].m_GPSPointVector.size() > 0)
				{
					...
					for (int local_l = 0; local_l < m_GridMatrix[x_i][y_i].m_LinkNoVector.size(); local_l++) // for all links in this cell
					{
						...
						if (g_link_vector[l].link_id == 1117)
							TRACE("");
				...
				for (int g = 0; g < m_GridMatrix[x_i][y_i].m_GPSPointVector.size(); g++) // for each GPS point of an agent in the cell
				{
					...
					for (int local_l = 0; local_l < m_GridMatrix[x_i][y_i].m_LinkNoVector.size(); local_l++) // for all links in this cell
					{
						...
						if (g_link_vector[l].from_node_id == 287703 && g_link_vector[l].to_node_id == 348503)
							TRACE("%d->%d\n", g_link_vector[l].from_node_id, g_link_vector[l].to_node_id);
					...
					if(g_link_vector[l].m_PointVector.size()>=2)  // protection
					{
						...
						for (int p = 0; p < g_link_vector[l].m_PointVector.size(); p++)
						{
							...
							{
								nonhit_distance += g_link_vector[l].link_distance; // add some penalty of link distance if there is just one GPS point to be matched. 
	...
	void find_path_for_agents_assigned_for_this_thread()
	{
		...
		for (int i = 0; i < m_agent_vector.size(); i++)
		{
			...
			cout << "destination node id  =" << g_node_vector [ destination_node_no].node_id << endl;
...
void g_ReadInputData()
{
	...
	if (parser_link.OpenCSVFile("link.csv", true))
	{
		while (parser_link.ReadRecord())
		{
			...
			if (link.link_id == 7824)
			{
				TRACE("");
			}

			if (link.link_id == 7823)
			{
				TRACE("");
			}
			...
			if (link.from_node_id == 17700 && link.to_node_id == 17703)
			{
				TRACE("");
			}
			...
			Point = g_node_vector[link.from_node_seq_no].pt;
			Point_next = g_node_vector[link.to_node_seq_no].pt;
			...
			if (CoordinateVector.size() >= 2)
			{
				...
				for (int l = 0; l < CoordinateVector.size(); l++)
				{
					...
					if (l < CoordinateVector.size() - 1) // consider shape points in each segment of a link
					{
						...
						link_distance += g_GetPoint2Point_Distance(&Point, &Point_next);
					}
				}
...
void g_ReadTraceCSVFile()
{
	...
	if (gps_parser.OpenCSVFile("trace.csv", true))
	{
		...
		while (gps_parser.ReadRecord())
		{
			...
			gps_parser.GetValueByFieldName("i_node_id", i_node_id, false);
...
void g_OutputRouteCSVFile()
{
	...
	{
		fprintf(g_pFileLinkRoute, "agent_id,given_link_type_code,road_sequence,from_node_id,to_node_id,link_id,trace_no,trace_id,length,cumulative_distance,likelihood_distance,link_type_code,geometry\n");
		...
		for (int a = 0; a < g_agent_vector.size(); a++)
		{
			...
			{
				int link_no = g_node_vector[p_agent->path_node_vector[i]].m_outgoing_link_seq_no_map[p_agent->path_node_vector[i + 1]];
				//cout << "i = " << i << " to node id  =" << g_node_vector[p_agent->path_node_vector[i + 1]].node_id << endl;
				...
				// timestamp
				int trace_no = -1;
				trace_no = p_agent->likely_trace_no_vector[i];
				...
				if (trace_id.size() > 0)
					fprintf(g_pFileLinkRoute, "%s,", trace_id.c_str());
				else
				...
				fprintf(g_pFileLinkRoute, "%f,", g_link_vector[link_no].likelihood_distance);
				...
				if (g_link_vector[link_no].geometry.size() > 0)
				{
				...
				fprintf(g_pFileLinkRoute, "%f %f,", g_node_vector[g_link_vector[link_no].from_node_seq_no].pt.x, g_node_vector[g_link_vector[link_no].from_node_seq_no].pt.y);
				fprintf(g_pFileLinkRoute, "%f %f", g_node_vector[g_link_vector[link_no].to_node_seq_no].pt.x, g_node_vector[g_link_vector[link_no].to_node_seq_no].pt.y);
...
void g_OutputLinklikelihoodCSVFile()
{
	...
	{
		fprintf(g_pFileLinkRoute, "link_id,link_type_code,from_node_id,to_node_id,cell_id,FFTT_in_sec,likelihood_distance,o_distance,d_distance,accessbility_time,hit_count,use_count,balance,geometry\n");

		for (int a = 0; a < g_link_vector.size(); a++)
		{
			if (g_link_vector[a].from_node_id == 17700 && g_link_vector[a].to_node_id == 17703)
			{
				TRACE("");
			}
			if (g_link_vector[a].likelihood_distance < 998)
			{
				...
				fprintf(g_pFileLinkRoute, "%d,%s,%d,%d,%jd,%d,%f,%f,%f,%d,%d,%d,%f,%f,", g_link_vector[a].link_id, 
					g_link_vector[a].link_type_code.c_str(),
					from_node_id, to_node_id, g_link_vector[a].cell_id, g_link_vector[a].FFTT_in_sec,
