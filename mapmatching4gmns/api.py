...
Engine 1 (HMM / native): Zhou's `trace2route` most-likely-path matcher, built as an in-process
                          pybind module (native/). Returns a routable, connected link path.
...
def engines_available(network_dir=None):
    """Which engines can run here. HMM needs the native module (or trace2route.exe)."""
    return {"hmm_native": _hmm.available(), "geometric": True}
...
def match(evidence, network_dir=None, base_link_df=None, gp_types=("1", "2", "3"),
    ...
    engine: 'hmm' (native trace2route; needs network_dir),
            'geometric' (needs network_dir or base_link_df),
    ...
    raise ValueError("engine must be 'hmm', 'geometric', or 'both'")
